<script lang="ts">
	...
	function randomTransform({ translateX, translateY, rotateZ }: TransformOpts) {
		return [
			`translateX(${random.number({ diff: translateX.degree * 0.5, base: translateX.degree * 0.5 })}${translateX.unit})`,
...
<div bind:this={el} />
<section style:background-color={backgroundColor} style:transform={transform}>
    <slot />
</section>
